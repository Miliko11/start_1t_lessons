from PyQt5.QtWidgets import (QApplication, QMainWindow, QLabel, QMessageBox,
                             ...
                             QComboBox, QHBoxLayout, QAction, QSizePolicy, QRadioButton)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        with open("fuel.txt", "r", encoding="utf-8") as file:
            ...
            for line in lines:  # Проходимся по каждой строчке
                ...
                self.fuel.update({key: value_list})
            print(self.fuel)
        self.undo_stack = []
    ...
    def init_ui(self):
        ...
        for key in self.fuel.keys():
            self.combo_box_fuel.addItem(str(key))
        ...
        self.discount = [0, 1, 2, 3, 5, 10, 12, 20]
        ...
        self.radio_payment_nal = QRadioButton("Наличные", self)
        self.radio_payment_bank = QRadioButton("Картой", self)
        ...
        self.layout_main.addWidget(self.radio_payment_nal)
        self.layout_main.addWidget(self.radio_payment_bank)
        ...
        self.edit_menu = self.menuBar().addMenu("Отмена")
        ...
        self.setStyleSheet("""
            ...
            QPushButton:hover{background-color: #FF7F50; color: black; font-weight: bold;}
            QLabel
            {
                color: black;
                font-size: 20px;
    ...
    def set_cost(self):
        self.cost = get_cost(float(self.input_price.text()),
                             ...
                             int(self.input_number.text()))
    def trade(self):
        if self.radio_payment_nal.isChecked() == False and self.radio_payment_bank.isChecked() == False:
            QMessageBox.information(self, "Оплата", "Не выбран вид оплаты")
        else:
            if self.check():
                ...
                if self.radio_payment_bank.isChecked():
                    self.payment = "Банк"
                else:
                    self.payment = "Наличные"
                self.undo_stack.append([self.combo_box_fuel.currentText(), float(self.input_price.text()),
                                        int(self.combo_box_discont.currentText()[:-1]),
                                        int(self.input_number.text()), round(self.cost, 2), self.payment])
    ...
    def cancel_trade(self):
        if self.undo_stack:
            ...
            self.lbl_itog.setText("Итого за смену: " + str(self.total) + " руб.")
            if str(z[5]) == "Наличные":
                self.radio_payment_nal.click()
            else:
                self.radio_payment_bank.click()
            self.undo_stack.pop()
            self.count_receipt -= 1
            self.btn_trade.setText("Чек № А-" + str(self.count_receipt))
            self.update()
